...
export function SponsorsForm({ initialSponsors = [], initialPrizes = [], eventId }) {
  ...
  const cachedData = getCurrentSectionData("sponsors")

  // Initialize state with cached data or initial data
  const [sponsors, setSponsors] = useState(cachedData?.sponsors || initialSponsors || [])
  const [prizes, setPrizes] = useState(cachedData?.prizes || initialPrizes || [])
  ...
  const originalSponsorsRef = useRef(JSON.parse(JSON.stringify(initialSponsors || [])))
  const originalPrizesRef = useRef(JSON.parse(JSON.stringify(initialPrizes || [])))
  ...
  useEffect(() => {
    ...
    cacheFormData("sponsors", currentData)

    // Calculate changed fields
    const hasSponsorChanges = JSON.stringify(sponsors) !== JSON.stringify(originalSponsorsRef.current)
    const hasPrizeChanges = JSON.stringify(prizes) !== JSON.stringify(originalPrizesRef.current)

    const changedFields = {}
    if (hasSponsorChanges) changedFields.sponsors = sponsors
    if (hasPrizeChanges) changedFields.prizes = prizes

    setChangedFields("sponsors", changedFields)
  }, [sponsors, prizes, cacheFormData, setChangedFields])
  ...
  const handleContinue = () => {
    ...
    originalSponsorsRef.current = JSON.parse(JSON.stringify(sponsors))
    originalPrizesRef.current = JSON.parse(JSON.stringify(prizes))
  ...
  return (
    <FormWrapper section="sponsors" initialData={{ sponsors: initialSponsors || [], prizes: initialPrizes || [] }}>
      <motion.div
