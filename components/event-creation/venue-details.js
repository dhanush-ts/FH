...
export function VenueForm({ initialData, eventId }) {
  ...
  // Initialize form with cached data or initial data
  const form = useForm({
    resolver: zodResolver(venueSchema),
    defaultValues: {
      place: cachedData?.place || initialData?.place || "",
      address: cachedData?.address || initialData?.address || "",
      gmaps_link: cachedData?.gmaps_link || initialData?.gmaps_link || "",
    },
  })
  ...
  useEffect(() => {
    cacheFormData("venue", formValues)

    // Calculate changed fields
    const changedFields = getChangedFields(originalDataRef.current, formValues)
    setChangedFields("venue", changedFields)
  }, [formValues, cacheFormData, setChangedFields])
  ...
  return (
    <FormWrapper section="venue" initialData={originalDataRef.current}>
      <motion.div
        initial="hidden"
                      ...
                      >
                        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                          <Button
                            type="button"
                            variant="outline"
                            onClick={() => router.push(`/host/create/${eventId}/data`)}
